...
use std::{io::Cursor, env, process::exit};
...
use aws_sdk_sqs::{config::Region, meta::PKG_VERSION, Client};
...
async fn main() -> Result<(), Error> {
    ...
    let queue_name_ref = &queue_name;
        ...
        let func = service_fn(move |event| async move { function_handler(event,bucket_name_ref, s3_client_ref,sqs_client_ref,queue_name_ref).await });
